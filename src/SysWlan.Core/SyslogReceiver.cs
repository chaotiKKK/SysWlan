    ...
    private async Task ReceiveAsync(UdpClient socket, CancellationToken token)
    {
        ...
        try
        {
            while (!token.IsCancellationRequested)
            {
                ...
                store.AddLog(SyslogParser.Parse(Encoding.UTF8.GetString(packet.Buffer, 0, Math.Min(packet.Buffer.Length, 8192)), packet.RemoteEndPoint.Address.ToString(), DateTimeOffset.UtcNow));
            }
        }
