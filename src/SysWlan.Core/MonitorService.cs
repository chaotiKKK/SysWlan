...
public sealed class MonitorService(INetworkCollector collector, Store store, RouterProbe probe, DeviceTimelineService timeline) : IDisposable
{
    ...
    private async Task LoopAsync()
    {
        ...
        try
        {
            store.AddLog(new(DateTimeOffset.UtcNow, "App", "Info", "Monitoring gestartet. Datenmengen zählen nur beobachtete Zeiträume dieses PCs."));
            while (!token.IsCancellationRequested)
            {
                if (paused) { rates.Reset(); await Task.Delay(500, token); continue; }
                try
                {
                    ...
                    state = new() { Snapshot = snapshot, Router = router, Traffic = traffic, History = id is null ? [] : store.History(id), Profiles = store.Profiles(), Logs = store.Logs(), Findings = SecurityAnalyzer.Analyze(snapshot, router), Paused = paused };
                }
                catch (Exception ex) when (ex is not OperationCanceledException || !token.IsCancellationRequested)
                {
                    rates.Reset();
                    timeline.RecordCollectionError(DateTimeOffset.UtcNow, ex.Message);
                    state = state with { Error = "Erfassung fehlgeschlagen: " + SyslogParser.Redact(ex.Message), Paused = paused };
                }
                Changed?.Invoke();
